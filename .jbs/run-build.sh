...
export GRADLE_HOME=/opt/gradle/8.0.2
export TOOL_VERSION=8.0.2
export PROJECT_VERSION=3.1.5
export JAVA_HOME=/lib/jvm/java-11
export ENFORCE_VERSION=

set -- "$@" assemble publishToMavenLocal -DdisableTests -Prelease 
...
if [ -n "" ]; then
  echo "Creating tag 3.1.5 to match enforced version"
  git tag -m 3.1.5 -a 3.1.5 || true
fi
